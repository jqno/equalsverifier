...
public class CachingValueProviderTest {
    ...
    void useFallbackValueAndCheckCache() {
        assertThat(sut.provide(new TypeTag(int.class), SOME_FIELD)).contains(new Tuple<>(42, 1337, 42));
    }

    @Test
    void useCachedValue() {
        cache.put(SOME_FIELD, new Tuple<>(1, 2, 1));
        assertThat(sut.provide(new TypeTag(int.class), SOME_FIELD)).contains(new Tuple<>(1, 2, 1));
    }

    @Test
    void dontUseCachedValueForOtherField() {
        cache.put(SOME_FIELD, new Tuple<>("a", "b", "a"));
        assertThat(sut.provide(new TypeTag(String.class), "somethingElse")).isEmpty();
    }

    @Test
    void dontUseCachedValueForNullField() {
        cache.put(null, new Tuple<>("a", "b", "a"));
