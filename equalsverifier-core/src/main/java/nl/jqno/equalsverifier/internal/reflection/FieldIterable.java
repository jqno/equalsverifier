...
public final class FieldIterable implements Iterable<FieldProbe> {
    ...
    private List<FieldProbe> addFieldsFor(Class<?> c) {
        ...
        for (Field field : c.getDeclaredFields()) {
            if (field.isSynthetic() && !allowedSynthetic(field)) {
                continue;
            }
            if (disallowedNonsynthetic(field)) {
                continue;
            }
    ...
    private boolean allowedSynthetic(Field field) {
        return field.getName().startsWith("$$delegate_");
    }

    private boolean disallowedNonsynthetic(Field field) {
        return field.getName().startsWith("bitmap$init$") // Generated by Scala 2.x's -Xcheckinit flag
                || field.getName().equals("__cobertura_counters");
    }
}
